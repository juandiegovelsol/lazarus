var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
// Define an enum for transaction categories
var TransactionCategory;
(function (TransactionCategory) {
    TransactionCategory["Income"] = "Income";
    TransactionCategory["Food"] = "Food";
    TransactionCategory["Rent"] = "Rent";
    TransactionCategory["Uncategorized"] = "Uncategorized";
})(TransactionCategory || (TransactionCategory = {}));
var PersonalFinanceManager = /** @class */ (function () {
    function PersonalFinanceManager() {
        this.transactions = [];
        this.budgets = {};
        this.storageKey = 'userTransactions';
        this.loadTransactions();
    }
    // Load transactions from local storage
    PersonalFinanceManager.prototype.loadTransactions = function () {
        var data = localStorage.getItem(this.storageKey);
        if (data) {
            var storedTransactions = JSON.parse(data);
            this.transactions = storedTransactions.map(function (txn) { return (__assign(__assign({}, txn), { category: Object.values(TransactionCategory).includes(txn.category)
                    ? txn.category
                    : TransactionCategory.Uncategorized })); });
        }
    };
    // Add a new transaction
    PersonalFinanceManager.prototype.addTransaction = function (transaction) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            return __generator(this, function (_a) {
                this.transactions.push(transaction);
                // Simulate an asynchronous save operation
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        setTimeout(function () {
                            try {
                                _this.saveTransactions();
                                resolve();
                            }
                            catch (error) {
                                reject(error);
                            }
                        }, 1000);
                    })];
            });
        });
    };
    // Set budget for a category
    PersonalFinanceManager.prototype.setBudget = function (category, amount) {
        this.budgets[category] = amount;
    };
    // Check if budgets are exceeded and notify user
    PersonalFinanceManager.prototype.checkBudgets = function () {
        var spending = this.transactions.reduce(function (acc, txn) {
            // Only consider expenses (negative amounts)
            if (txn.amount < 0) {
                acc[txn.category] = (acc[txn.category] || 0) + Math.abs(txn.amount);
            }
            return acc;
        }, {});
        for (var category in this.budgets) {
            if (spending[category] > this.budgets[category]) {
                alert("Budget exceeded for category: ".concat(category));
            }
        }
    };
    // Save transactions to local storage (encrypted)
    PersonalFinanceManager.prototype.saveTransactions = function () {
        return __awaiter(this, void 0, void 0, function () {
            var key, encryptedData;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, crypto.subtle.generateKey({
                            name: 'AES-GCM',
                            length: 256,
                        }, true, ['encrypt', 'decrypt'])];
                    case 1:
                        key = _a.sent();
                        return [4 /*yield*/, crypto.subtle.encrypt({
                                name: 'AES-GCM',
                                iv: new Uint8Array(12), // Initialize vector
                            }, key, new TextEncoder().encode(JSON.stringify(this.transactions)))];
                    case 2:
                        encryptedData = _a.sent();
                        // Save the encrypted data to local storage
                        localStorage.setItem(this.storageKey, JSON.stringify(new Uint8Array(encryptedData)));
                        return [2 /*return*/];
                }
            });
        });
    };
    return PersonalFinanceManager;
}());
// Example Usage
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var financeManager;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    financeManager = new PersonalFinanceManager();
                    // Adding transactions
                    return [4 /*yield*/, financeManager.addTransaction({
                            id: 1,
                            description: 'Salary',
                            amount: 5000,
                            category: TransactionCategory.Income,
                        })];
                case 1:
                    // Adding transactions
                    _a.sent();
                    return [4 /*yield*/, financeManager.addTransaction({
                            id: 2,
                            description: 'Groceries',
                            amount: -150,
                            category: TransactionCategory.Food,
                        })];
                case 2:
                    _a.sent();
                    // Setting a budget
                    financeManager.setBudget(TransactionCategory.Food, 500);
                    // Checking budgets
                    financeManager.checkBudgets();
                    return [2 /*return*/];
            }
        });
    });
}
main();
