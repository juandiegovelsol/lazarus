...
const cursorTooltipTime = document.getElementById("cursor-tooltip-time");
const cursorTooltipPrice = document.getElementById("cursor-tooltip-price");
...
chart.addEventListener("mousemove", (event) => {
  // Calculate the position of the cursor relative to the chart
  const x = event.clientX - chart.offsetLeft;
  const y = event.clientY - chart.offsetTop;

  // Update the position of the cursor lines
  cursorLineX.style.top = `${y}px`;
  cursorLineY.style.left = `${x}px`;

  // Update the position and content of the cursor tooltip
  const time = Math.floor((x / (chart.offsetWidth - 30)) * 24);
  const price = Math.round(
    (1 - y / chart.offsetHeight) * (Math.max(...data) - Math.min(...data)) +
      Math.min(...data)
  );
  cursorTooltipTime.style.bottom = `${chart.offsetHeight}px`;
  cursorTooltipTime.style.left = `${x}px`;
  cursorTooltipTime.textContent = `${String(Math.floor(time)).padStart(
    2,
    "0"
  )}:00`;
  cursorTooltipPrice.style.left = `${chart.offsetWidth}px`;
  cursorTooltipPrice.style.top = `${y}px`;
  cursorTooltipPrice.textContent = price;
});
