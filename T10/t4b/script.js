...
function moveAdditionalCircle(index) {
  ...
  while (overlap) {
    ...
    // Check for overlap with the main circle and other additional circles
    if (isInsideCircle(circleX, circleY, newX, newY, circleRadius * 2)) {
      overlap = true;
    } else {
      for (let i = 0; i < additionalCircles.length; i++) {
        if (
...
function initializeAdditionalCircles() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    while (overlap) {
      ...
      // Check for overlap with the main circle and previously placed additional circles
      if (isInsideCircle(circleX, circleY, newX, newY, circleRadius * 2)) {
        overlap = true;
      } else {
        for (const additionalCircle of additionalCircles) {
