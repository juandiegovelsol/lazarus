...
function createShape() {
  ...
  // Saves the original scale as an HTML data attribute
  shape.dataset.originalScale = shapeScale;
...
function shapeClicked(shape, shapeType) {
  if (clickedShape === null) {
    ...
    const originalScale = shape.dataset.originalScale;
    shape.style.transform = `translate(-50%, -50%) scale(${originalScale})`;
    ...
    if (clickedShape.type === shapeType) {
      ...
      const originalScale = clickedShape.shape.dataset.originalScale;
      clickedShape.shape.style.transform = `translate(-50%, -50%) scale(${originalScale})`;
    }
