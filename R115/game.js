...
class Game {
  ...
  drawCard(player) {
    ...
    console.log(
      `Player ${player.id} drew a card with value ${drawnCard.value}`
    );
  }
  ...
  swapCard(player) {
    ...
    console.log(
      `Player ${player.id} swapped a card with the top card of the discard pile`
    );
  }
  ...
  discardCard(player) {
    ...
    console.log(
      `Player ${player.id} discarded a card with value ${discardedCard.value}`
    );
  }
  ...
  skrewRound() {
    ...
    const lowestPlayer = this.players.find(
      (player) => player.points === lowestPoints
    );
  ...
  gameOver() {
    ...
    const winner = this.players.reduce(
      (min, player) => (player.score < min.score ? player : min),
      this.players[0]
    );
