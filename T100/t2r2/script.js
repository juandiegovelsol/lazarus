// Get references to the chart container, y-axis, x-axis, and timeframe dropdown
...
function createCandles(data, timeframe) {
  ...
  for (let i = 0; i < data.length - timeframe; i += timeframe) {
    ...
    candle.style.bottom = `${
      (bottom / (Math.max(...data) - minPrice)) * chart.offsetHeight
    }px`;
    candle.style.height = `${
      (height / (Math.max(...data) - minPrice)) * chart.offsetHeight
    }px`;
...
/**
 * Updates the y-axis labels based on the price data.
 * @param {number[]} data - Array of price data points.
 */
function updateYAxis(data) {
  ...
  const numLabels = 10;
  const labelInterval = priceRange / numLabels;

  yAxis.innerHTML = "";

  for (let i = 0; i <= numLabels; i++) {
    const label = document.createElement("div");
    label.textContent = `${(maxPrice - i * labelInterval).toFixed(2)}`;
    label.style.flexBasis = `${(1 / numLabels) * 100}%`;
    yAxis.appendChild(label);
  }
}

/**
 * Updates the x-axis labels based on the data length and timeframe.
 * @param {number[]} data - Array of price data points.
 * @param {number} timeframe - Number of data points to group into one candle.
 */
function updateXAxis(data, timeframe) {
  const numLabels = Math.floor(data.length / timeframe);
  const labelInterval = numLabels > 10 ? Math.floor(numLabels / 10) : 1;

  xAxis.innerHTML = "";

  for (let i = 0; i <= numLabels; i += labelInterval) {
    const label = document.createElement("div");
    label.textContent = `${padZero(Math.floor((i * timeframe) / 12))}:${padZero(
      ((i * timeframe) % 12) * 5
    )}`;
    label.style.flexBasis = `${(1 / numLabels) * 100}%`;
...
// Update candles and axes on timeframe selection change
timeframeSelect.addEventListener("change", () => {
  const timeframeValue = parseInt(timeframeSelect.value);
  createCandles(data, timeframeValue);
  updateXAxis(data, timeframeValue);
});

// Initial rendering of candles and axes with default timeframe
