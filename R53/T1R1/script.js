document.addEventListener("DOMContentLoaded", function () {
  ...
  let allCryptocurrencies = []; // Array to store all cryptocurrencies
  ...
  function renderAutofillSuggestions(cryptocurrencies) {
    ...
    for (let crypto of cryptocurrencies) {
      ...
      autofillItem.classList.add("autofill-item");
      autofillItem.innerHTML = `${crypto.name} (${crypto.symbol})`;
      autofillItem.addEventListener("click", () => {
        cryptoIdInput.value = crypto.id;
        autofillContainer.innerHTML = "";
      });

      autofillContainer.appendChild(autofillItem);
    }
  }
  ...
  cryptoIdInput.addEventListener("input", async function () {
    ...
    const filteredCryptocurrencies = allCryptocurrencies.filter(
      (crypto) =>
        crypto.name.toLowerCase().includes(inputValue) ||
        crypto.symbol.toLowerCase().includes(inputValue)
    );

    renderAutofillSuggestions(filteredCryptocurrencies.slice(0, 5));
  });
  ...
  // Fetch all cryptocurrencies and store them in the allCryptocurrencies array
  fetchAllCryptocurrencies().then((data) => {
