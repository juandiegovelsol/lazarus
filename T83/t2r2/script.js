...
let isDifficultyLevelChanged = false; // Flag to track if the difficulty level has been changed with the mouse
...
window.addEventListener("keydown", (event) => {
  ...
  // Prevent changing difficulty level with arrow keys if it has been changed with the mouse
  if (isDifficultyLevelChanged && event.key.startsWith("Arrow")) {
...
difficultyLevelSelect.addEventListener("change", (event) => {
  ...
  isDifficultyLevelChanged = true; // Set flag to true
  clearInterval(gameIntervalId);
