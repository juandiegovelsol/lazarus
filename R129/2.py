class InsurancePolicy:
    def __init__(self, age, health_score, vehicle_type, accident_history, location, policy_duration, credit_score, employment_status, annual_income):
        ...
        self.accident_history = self._validate_accident_history(accident_history)
        ...
        self.employment_status = self._validate_employment_status(employment_status)
    ...
    def _validate_vehicle_type(self, vehicle_type):
        ...
        if vehicle_type not in valid_types:
            raise ValueError("Invalid vehicle type. Must be one of: {}".format(", ".join(valid_types)))
        return vehicle_type
    ...
    def _validate_location(self, location):
        ...
        if location not in valid_locations:
            raise ValueError("Invalid location. Must be one of: {}".format(", ".join(valid_locations)))
        return location
    ...
    def _validate_employment_status(self, employment_status):
        ...
        if employment_status not in valid_statuses:
            raise ValueError("Invalid employment status. Must be one of: {}".format(", ".join(valid_statuses)))
    ...
    def calculate_premium(self):
        ...
        premium = self._apply_health_score_and_accident_history_adjustment(premium)
        premium = self._calculate_tiered_discount(premium)
        return max(premium, 0)  # Ensure premium is not negative

# Example usage
policy = InsurancePolicy(24, 50, "sports", 3, "urban", 3, 750, "employed", 70000)
