...
// Calculate total percentage to adjust for 100% ring coverage
const totalPercentage = files.reduce((acc, file) => acc + file.percentage, 0);
...
function createRingChart(files) {
  ...
  files.forEach((file, index) => {
    ...
    const adjustedPercentage = (file.percentage / totalPercentage) * 100;
    ...
    segment.setAttribute(
      "stroke-dasharray",
      `${(adjustedPercentage / 100) * circumference} ${circumference}`
    );
