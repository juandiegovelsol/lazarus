let startButton, resetButton, predictButton;
...
function setup() {
  ...
  predictButton = createButton('Show Predictions');
  predictButton.position(360, height + 20);
  predictButton.mousePressed(togglePredictions);

  // Perceptron layout
  createElement('h2', 'Perceptron Layout:').position(550, 20);
  createElement('p', 'Input 1:').position(550, 50);
  createElement('p', 'Input 2:').position(550, 80);
  createElement('p', 'Bias:').position(550, 110);
  createElement('p', 'Output:').position(550, 140);
}
...
function togglePredictions() {
  ...
  if (showPredictions) {
    predictButton.html('Hide Predictions');
  } else {
    predictButton.html('Show Predictions');
  }
}
...
function plotData(data) {
    ...
    data.forEach(d => {
        ...
        let py = map(d.y + 0.025, 0, 1, height, 0);
...
function displayPerceptronLayout() {
  fill(0);
  textSize(16);
  textAlign(LEFT);
  text(`Weight 1: ${w1.toFixed(4)}`, 550, 70);
  text(`Weight 2: ${w2.toFixed(4)}`, 550, 100);
  text(`Bias: ${bias.toFixed(4)}`, 550, 130);
  let output = predict(0.5, 0.5);
  text(`Output: ${output.toFixed(4)}`, 550, 160);
}