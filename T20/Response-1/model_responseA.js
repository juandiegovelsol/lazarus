...
function generateXORData(numPointsPerRegion) {
    ...
    const step = 2 / numPointsPerSide; // changed to 2 to cover the range of -1 to 1
    ...
    for (let i = 0; i < numPointsPerSide; i++) {
        for (let j = 0; j < numPointsPerSide; j++) {
            const x = i * step - 1 + step; // adjusted to maintain the step and prevent points from overlapping
            const y = -1 + j * step;
            points.push({ x: x, y: y, label: 1 });
        }
    }
...
function plotData(data) {
    ...
    data.forEach(d => {
        let px = map(d.x, -1, 1, 40, width - 40);
        let py = map(d.y, -1, 1, height - 40, 40);
...
function updateDecisionBoundary() {
    ...
    // Midpoint at (0, 0), weights are used to calculate slope
    let midpointX = 0;
    let midpointY = 0;
    let slope = -(w1 / w2);
