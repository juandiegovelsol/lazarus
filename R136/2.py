...
class POS:
    ...
    def pay_employee(self, amount):
        ...
        if self.cash_shortage_account.balance != 0:
            # Clear the shortage account
            part_tran3 = PartTran(abs(self.cash_shortage_account.balance), self.cash_shortage_account,
                                  'Clearing shortages', 'D')
            part_tran4 = PartTran(abs(self.cash_shortage_account.balance), self.employee_payable_account,
                                  'Recovering shortages', 'C')
...
def main():
    ...
    pos.perform_end_of_day(1, datetime.today())
