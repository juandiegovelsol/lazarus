...
class POS:
    ...
    def sell_item(self, item: Item, quantity: int):
        ...
        part_tran3 = PartTran(item.selling_price * quantity, self.sales_revenue_account, f"Sale of {item.name}", 'C')
        part_tran4 = PartTran(item.selling_price * quantity, self.cash_account, f"Sale of {item.name}", 'D')
    ...
    def handle_expenses(self, expense_name: str, amount: float):
        part_tran1 = PartTran(amount, self.cash_account, expense_name, "C")
        part_tran2 = PartTran(amount, self.expense_account, expense_name, "D")
    ...
    def perform_end_of_day(self, cash_in_cash_box: float, date: date):
        ...
        if cash_in_cash_box < abs(self.cash_account.balance):
            # Ask if the shortage should be recorded for future employee salary deduction
            response = input(
                f"There is a shortage of {abs(cash_in_cash_box - abs(self.cash_account.balance))}. Do you want to add it to shortage for future employee salary deduction? Y/N: ")

            if response == 'Y' or response == 'y':
                # Record the cash shortage as a liability
                part_tran1 = PartTran(abs(cash_in_cash_box - abs(self.cash_account.balance)), self.cash_account,
                                      'Recording a shortage', 'C')
                part_tran2 = PartTran(abs(cash_in_cash_box - abs(self.cash_account.balance)), self.cash_shortage_account,
    ...
    def pay_employee(self, amount):
        ...
        part_tran1 = PartTran(amount, self.cash_account, 'Employee payment', 'D')
        part_tran2 = PartTran(amount, self.employee_payable_account, 'Employee payment', 'C')
        tran_header = TranHeader(datetime.today(), [part_tran1, part_tran2])

        # If there are any cash shortages, record them as part of the employee payment
        if self.cash_shortage_account.balance != 0:
            part_tran3 = PartTran(abs(self.cash_shortage_account.balance), self.cash_shortage_account, 'Recovering shortages', 'C')
            part_tran4 = PartTran(abs(self.cash_shortage_account.balance), self.employee_payable_account, 'Recovering shortages', 'D')
