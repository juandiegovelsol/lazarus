...
class Ball {
  constructor(x, y, radius, color) {
    ...
    this.dx = (Math.random() - 0.5) * 2; // Apply wind in the NW direction
    this.dy = 0;
    this.bounces = 0;
  }
  ...
  update() {
    this.dy += gravity;
    this.dy *= friction;
    this.dx *= friction; // Apply friction in the x-direction

    this.x += this.dx + 3 / 60; // Apply wind flow of 3 m/s in the NW direction
    this.y += this.dy;
...
for (let i = 0; i < 100; i++) {
  ...
  const y = Math.random() * 100 + radius; // Balls start from the top of the screen
  const color = "blue";
