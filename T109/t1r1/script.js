...
function createChart(data) {
  ...
  data.forEach((value) => {
    ...
    const height = Math.abs((value / maxValue) * 50); // 50% of the container height
    bar.style.height = `${height}%`;

    // Position bars above or below the center line
    bar.style.transform = `translateY(${value < 0 ? "50%" : "-50%"}); scaleY(${
      height / 50
    })`; // Scale the bar to fit within the container
