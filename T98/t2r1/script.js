...
function initializeStandings() {
  ...
  teams.forEach((team) => {
    standingsData[team] = {
      ...
      drawn: 0,
      lost: 0,
      goalsDiff: 0, // Initialize goals difference
    };
  });
...
function updateStandings(match) {
  ...
  standingsData[homeTeam].goalsDiff += homeGoals - awayGoals;
  standingsData[awayTeam].goalsDiff += awayGoals - homeGoals;
...
function updateStandingsDisplay() {
  ...
  const sortedTeams = Object.keys(standingsData).sort((a, b) => {
    if (standingsData[b].points === standingsData[a].points) {
      return standingsData[b].goalsDiff - standingsData[a].goalsDiff;
    }
    return standingsData[b].points - standingsData[a].points;
  });
  sortedTeams.forEach((team) => {
    const row = document.createElement("tr");
    const { points, played, won, drawn, lost, goalsDiff } = standingsData[team];
    row.innerHTML = `<td>${team}</td><td>${points}</td><td>${played}</td><td>${won}</td><td>${drawn}</td><td>${lost}</td><td>${goalsDiff}</td>`;
...
function updateMatchTable(match, index) {
  ...
  const homeTeamCell = row.cells[0]; // Get the home team cell
  const awayTeamCell = row.cells[1]; // Get the away team cell
  const matchResultCell = row.querySelector(".match-result"); // Get the result cell of the current match

  // Color the winner, loser or both in case of a draw
  if (match.homeGoals > match.awayGoals) {
    homeTeamCell.style.background = "lightgreen";
    awayTeamCell.style.background = "lightcoral";
  } else if (match.homeGoals < match.awayGoals) {
    homeTeamCell.style.background = "lightcoral";
    awayTeamCell.style.background = "lightgreen";
  } else {
    homeTeamCell.style.background = "lightgray";
    awayTeamCell.style.background = "lightgray";
  }
