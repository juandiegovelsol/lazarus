      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell>
              <TableSortLabel
                active={orderBy === "lowerAmount"}
                direction={orderBy === "lowerAmount" ? order : "asc"}
                onClick={() => handleSort("lowerAmount")}
              >
                Deposit Amount
              </TableSortLabel>
            </TableCell>
          ...
          {tableData.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell>
                {(
                  row.lowerAmount +
                  (row.lowerAmount * row.interestRate) / 100
                ).toFixed(2)}{" "}
                -{" "}
                {row.upperAmount
                  ? (
                      row.upperAmount +
                      (row.upperAmount * row.interestRate) / 100
                    ).toFixed(2)
                  : "No upper limit"}
              </TableCell>
            </TableRow>
