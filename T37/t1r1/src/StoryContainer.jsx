...
const StoryContainer = () => {
  ...
  const [firstOptionChosen, setFirstOptionChosen] = useState(true);

  const makeChoice = (choice) => {
    const nextPart = story[choice];

    setPreviousOptions((prevOptions) => [...prevOptions, choice]);

    if (firstOptionChosen) {
      setFirstOptionChosen(false);
    }

    setCurrentStory(choice);
  };

  const handleRestart = () => {
    setCurrentStory(1);
    setPreviousOptions([]);
    setFirstOptionChosen(true);
  };
  ...
  return (
    <div className="story-container">
      {previousOptions.map((option, index) => (
        <QuestionContainer
          key={index}
          text={story[option].text}
          options={story[option].options}
      ...
      <QuestionContainer
        ...
        options={story[currentStory].options}
        makeChoice={makeChoice}
        previousOptions={previousOptions}
        firstOptionChosen={firstOptionChosen}
      />
      {story[currentStory].options.length === 0 && (
