...
PLAYER1_UNITS = [
    {'type': 'General', 'x': 0, 'y': BOARD_HEIGHT // 2, 'movement': 2},
]
PLAYER2_UNITS = [
    {'type': 'General', 'x': BOARD_WIDTH - 1, 'y': BOARD_HEIGHT // 2, 'movement': 2},
]
...
while True:
                    ...
                    for y in range(BOARD_HEIGHT):
                        if (x * CELL_SIZE < event.pos[0] < (x + 1) * CELL_SIZE and
                            ...
                            if not any(u['x'] == x and u['y'] == y for u in units):
                                units.append({'type': DRAGGING_CARD['type'], 'x': x, 'y': y, 'movement': 2})
                        ...
                        if (x * CELL_SIZE < event.pos[0] < (x + 1) * CELL_SIZE and
                            ...
                            for enemy_unit in enemy_units:
                                if enemy_unit['x'] == x and enemy_unit['y'] == y:
                                    if enemy_unit['type'] == DRAGGING_UNIT['type']:
    ...
    for unit in PLAYER1_UNITS:
        ...
        text = FONT.render(str(unit['movement']), True, BLACK)
        WINDOW.blit(text, (unit['x'] * CELL_SIZE, unit['y'] * CELL_SIZE + CELL_SIZE))
